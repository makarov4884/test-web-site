...
const UPDATE_INTERVAL = 2 * 60 * 1000; // 2 minutes (Reduced from 5) for faster updates

// isUpdating moved near usage for clarity
...
let lockTime = 0;
const LOCK_TIMEOUT = 5 * 60 * 1000; // 5 minutes max lock
...
export async function updateNoticesCache(force: boolean = false): Promise<void> {
    if (isUpdating) {
        ...
        if (Date.now() - lockTime > LOCK_TIMEOUT) {
            console.log('[Notice Cache] ⚠️ Lock is stale (stuck for >10m). Resetting lock and proceeding.');
            isUpdating = false;
        } else {
            console.log('[Notice Cache] Update already in progress (Locked).');
            return;
        }
    }
    ...
    try {
        ...
        // Pass a callback to save data incrementally (every batch)
        const freshNotices = await crawlNoticesFromCrawler(async (partialNotices) => {
            if (partialNotices.length > 0) {
                const cachedNotices: CachedNotice[] = partialNotices.map(n => ({
                    ...n,
                ...
                console.log(`[Notice Cache] Incremental save: ${cachedNotices.length} notices.`);
            }
        });
