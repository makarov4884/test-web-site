    ...
    return (
        <header className="sticky top-0 z-50 w-full border-b border-pink-200/30 bg-white/70 backdrop-blur-md supports-[backdrop-filter]:bg-white/50">
            <div className="container mx-auto px-4 h-16 flex items-center justify-between">
                ...
                {/* Desktop Nav */}
                <nav className="hidden md:flex items-center space-x-0.5 lg:space-x-1 flex-1 justify-center max-w-2xl px-2">
                    {navItems.map((item) => (
                        <Link
                            key={item.name}
                            href={item.href}
                            className="px-2 lg:px-4 py-2 text-sm font-medium text-gray-600 hover:text-pink-600 hover:bg-pink-50/50 rounded-md transition-all whitespace-nowrap"
                        >
                            {item.name}
                        </Link>
                    ))}
