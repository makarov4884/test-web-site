...
export default function SchedulePage() {
    ...
    const [streamerColors, setStreamerColors] = useState<Record<string, { bg: string, text: string }>>({});
    ...
    useEffect(() => {
        ...
        const loadStreamersAndAuth = async () => {
            const session = await getSession();
            setUser(session);

            const streamersData = await getStreamers();

            // Build Color Map (key: bjId AND name to be safe, authorId might be nickname)
            const colorMap: Record<string, { bg: string, text: string }> = {};
            streamersData.forEach((s: any) => {
                if (s.color) {
                    colorMap[s.bjId] = s.color;
                    colorMap[s.name] = s.color; // Map name too in case authorId is nickname
                }
            });
            setStreamerColors(colorMap);

            if (session) {
                const isRegistered = streamersData.some((s: any) => s.bjId === session.id || s.id === session.id);
                setCanEdit(isRegistered);
            }
        };

        loadStreamersAndAuth();
    }, []);
                ...
                >
                    ...
                    <div className="mt-2 space-y-1">
                        {daySchedules.slice(0, 3).map((s, idx) => {
                            // Determine Color
                            let colorClass = 'bg-cyan-100 text-cyan-700';
                            if (s.type === 'rest') colorClass = 'bg-gray-200 text-gray-600';
                            else if (s.type === 'event') colorClass = 'bg-purple-100 text-purple-700';
                            else {
                                // For 'normal' broadcasts, use Streamer Color if available
                                const sColor = streamerColors[s.authorId];
                                if (sColor) colorClass = `${sColor.bg} ${sColor.text}`;
                            }
                            ...
                            <div className="space-y-3">
                                ...
                                {selectedDaySchedules.length > 0 ? (
                                    selectedDaySchedules.map(item => {
                                        // Determine Color for list
                                        let colorClass = 'bg-cyan-500'; // Default bullet
                                        let textClass = 'text-cyan-700'; // Default label check? No, used for bullet.

                                        // We need to style the container or the bullet.
                                        // Existing code uses a bullet color. Let's keep that but maybe match the text style or border?
                                        // Actually the user asked for "marked separately", so custom background items are nice.
                                        // But the list items currently are white boxes with colored bullets.

                                        let bulletColor = 'bg-cyan-500';
                                        if (item.type === 'rest') bulletColor = 'bg-gray-400';
                                        else if (item.type === 'event') bulletColor = 'bg-purple-500';
                                        else {
                                            const sColor = streamerColors[item.authorId];
                                            if (sColor) {
                                                // Convert Tailwind bg to a solid color if possible? Hard.
                                                // Let's just use the bg class for the bullet, assuming it works (it won't, bg-blue-100 is pale).
                                                // We need the "700" or "500" version.
                                                // Hack: sColor.text is 'text-blue-700'. We can extract 'blue-700' and make it 'bg-blue-500'.
                                                // Or just leave the bullet as cyan/purple/gray for type indication, 
                                                // AND add a colored badge for the name.
                                            }
                                        }
                                        ...
                                        return (
                                            <div key={item.id} className="p-4 rounded-xl bg-white border border-pink-200 flex items-center justify-between group shadow-sm">
                                                <div className="flex items-center gap-3">
                                                    <div className={cn("w-2 h-10 rounded-full", bulletColor)}></div>
                                                    <div>
                                                        ...
                                                        <div className="text-xs text-gray-500 flex gap-2">
                                                            ...
                                                            <span className={cn(
                                                                "font-bold",
                                                                item.type === 'normal' && streamerColors[item.authorId] ? streamerColors[item.authorId].text : ""
                                                            )}>
                                                                {item.authorId}
                                                            </span>
                                                        </div>
                                        ...
                                        <div key={item.id} className="p-4 rounded-xl bg-zinc-900 border border-white/5 flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-xs text-muted-foreground flex gap-2">
                                                    ...
                                                    <span className={cn(
                                                        "font-bold",
                                                        item.type === 'normal' && streamerColors[item.authorId] ? streamerColors[item.authorId].text.replace('700', '400') : "" // Lighten for dark mode
                                                    )}>
                                                        {item.authorId}
                                                    </span>
                                                </div>
